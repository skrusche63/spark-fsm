...
object TrainApp extends SparkService {
  ...
  protected val sc = createCtxLocal("SeriesContext",Configuration.spark)      
  protected val system = ActorSystem("SeriesSystem")
  ...
  protected def createParams(args:Array[String]):Map[String,String] = {
    ...
    val parser = new ArgotParser(
      programName = "Series Analysis Engine",
      compactUsage = true,
    ...
    config.value match {
      ...
      case Some(value) => {
        ...
        if (Algorithms.isAlgorithm(algo) == false)
          parser.usage("Parameter 'algo' must be one of [SPADE, TSR].")
          
        params += "algorithm" -> algo        
        params += "source" -> cfg.getString("source")

        /* SPADE */
        params += "support" -> cfg.getDouble("support").toString
...
class Handler(@transient ctx:RequestContext) extends Actor {
  ...
  def receive = {
    ...
    case req:ServiceRequest => {
      ...
      val master = context.actorOf(Props(new FSMMaster(ctx))) 
